...
var selectionKeys = contextKeys{
	...
	keys: []keyHint{
		...
		{"c", "colour", "Cycle all of them to one colour, the one the first gathered card steps to"},
		{"C", "pick", "Set one colour on all of them, chosen in the picker"},
		{"s", "size", "Cycle all of them to one card size, the one the first gathered card steps to"},

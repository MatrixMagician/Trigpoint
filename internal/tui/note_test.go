...
func TestANoteCardIsNeverWiderThanACard(t *testing.T) {
	for _, body := range []string{
		...
		for _, line := range card(state.Node{Kind: state.KindNote, Title: "todo", Note: body}, false, false, maxNoteLines, noteLines(body)) {
...
func TestANoteCardCarriesNoBadge(t *testing.T) {
	note := card(state.Node{Kind: state.KindNote, Title: "todo"}, false, false, 0, nil)
	if strings.Contains(note[0], "●") {
		t.Errorf("a note is never alive or dead, so it carries no badge: %q", note[0])
	}
	shell := card(state.Node{Kind: state.KindShell, Title: "api"}, false, false, 0, nil)

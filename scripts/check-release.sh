...
# it does.
#
#   usage: scripts/check-release.sh [dist-dir]
#
# dist-dir may be relative to the repo root or absolute. Needs docker or
# podman.
set -euo pipefail
...
dist=$(cd "${1:-dist}" && pwd)
tarball=$(ls "$dist"/*amd64.tar.gz)

docker run --rm -v "$dist:/dist:ro,z" debian:bookworm-slim sh -c '
	set -e
...
'
echo "ok: $(basename "$tarball") runs trig doctor on a clean machine"
